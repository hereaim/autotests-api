...
def assert_update_course_response(
    ...
    assert_equal(request.description, response.course.description,
                 'description')
    assert_equal(request.estimated_time, response.course.estimated_time,
                 'estimated_time')
...
def assert_get_courses_response(
    ...
    for index, create_course_responses in enumerate(create_course_responses):
        assert_course(get_courses_response.courses[index],
...
def assert_create_course_response(request: CreateCourseRequestSchema,
    ...
    assert_equal(response.course.description, request.description,
                 'description')
    assert_equal(response.course.estimated_time, request.estimated_time,
                 'estimated_time')
    assert_equal(response.course.preview_file.id, request.preview_file_id,
                 'preview_file_id')
    assert_equal(response.course.created_by_user.id, request.created_by_user_id,
                 'created_by_user_id')
