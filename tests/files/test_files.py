...
from clients.files.files_schema import CreateFileResponseSchema, \
    CreateFileRequestSchema, GetFileResponseSchema
...
from tools.assertions.files import assert_create_file_response, \
    assert_get_file_response, assert_create_file_with_empty_directory_response, \
...
class TestFiles:
    def test_create_file(self, files_client: FilesClient):
        ...
        response_data = CreateFileResponseSchema.model_validate_json(
            response.text)
    ...
    def test_get_file(self, files_client: FilesClient,
                      function_file: FileFixture):
        response = files_client.get_file_api(function_file.response.file.id)
        response_data = GetFileResponseSchema.model_validate_json(
            response.text)
    ...
    def test_create_file_with_empty_filename(self, files_client: FilesClient):
        request = CreateFileRequestSchema(
            ...
            upload_file="./testdata/files/image.png")
        response = files_client.create_file_api(request)
        response_data = ValidationErrorResponseSchema.model_validate_json(
            response.text)
    ...
    def test_delete_files(self, files_client: FilesClient,
                          function_file: FileFixture):
        delete_response = files_client.delete_file_api(
            function_file.response.file.id)
        assert_status_code(delete_response.status_code, HTTPStatus.OK)

        get_response = files_client.get_file_api(
            function_file.response.file.id)
